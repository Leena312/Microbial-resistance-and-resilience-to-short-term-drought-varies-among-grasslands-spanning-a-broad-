...
pdf(file="Shannon Diversity chapter 4 all",
    width=10,
    height=8)
...
ControlSGSsimp <- diversity(t(ControlSGS$data_loaded), index = "simpson")
ControlCHYsimp <- diversity(t(ControlCHY$data_loaded), index = "simpson")
ControlHYSsimp <- diversity(t(ControlHYS$data_loaded), index = "simpson")
ControlKNZsimp <- diversity(t(ControlKNZ$data_loaded), index = "simpson")

DroughtSGSsimp <- diversity(t(DroughtSGS$data_loaded), index = "simpson")
DroughtCHYsimp <- diversity(t(DroughtCHY$data_loaded), index = "simpson")
DroughtHYSsimp <- diversity(t(DroughtHYS$data_loaded), index = "simpson")
DroughtKNZsimp <- diversity(t(DroughtKNZ$data_loaded), index = "simpson")

colors<-c("blue","orange","blue","orange","blue","orange","blue","orange")

pdf(file="Simpson Diversity chapter 4 all",
    width=10,
    height=8)
a<-boxplot(ControlSGSsimp, DroughtSGSsimp, ControlCHYsimp, DroughtCHYsimp, ControlHYSsimp, DroughtHYSsimp, ControlKNZsimp, DroughtKNZsimp, col=colors, main = "Treatment",ylim=c(0.7,1), ylab = "Shannon's Diversity",xlab = "Treament", 
           names = c("SGS-C", "SGS-D", "CHY-C","CHY-D","HYS-C","HYS-D","KNZ-C","KNZ-D"))
dev.off()

allshan <- diversity(t(controldrought6$data_loaded), index = "simpson")
...
emmeans(allshan_glm,pairwise~Treatment|Site)
...
dm = calc_dm(SGS$data_loaded)
ord = calc_ordination(dm, 'nmds')
plot_ordination(SGS, ord, 'Treatment',  hulls = TRUE,)
library(ggplot2)
library(vegan)
...
Ordination.model2 <- CAPdiscrim (o.dist ~ Treatment, data = mta,
...
plot1 <- ordiplot(Ordination.model2, type="none")
ordisymbol(plot1, mta,"Treatment", legend=TRUE,legend.x="topleft")
ordiellipse(Ordination.model1, groups = mta$Treatment, draw = "polygon",col=c("blue","orange"))
dev.off()


SGScaps <- as.data.frame(Ordination.model1[["PCoA"]]) 
SGScaps$Treatment <- SGS$map_loaded$Treatment
KNZcaps <- as.data.frame(Ordination.model2[["PCoA"]]) 
KNZcaps$Treatment <- KNZ$map_loaded$Treatment

d<-ggplot(data=KNZcaps,aes(x=V1,y=V2,group=factor(Treatment),color=factor(Treatment))) + 
  xlab("LD1")+ylab("LD2")+
  stat_ellipse(data=KNZcaps,aes(x=V1,y=V2,group=factor(Treatment),color=factor(Treatment)),level=0.5) + # add the convex hulls
  ggtitle("KNZ")+
  geom_point(data=KNZcaps,aes(x=V1,y=V2,group=factor(Treatment),color=factor(Treatment))) + # add the point markers
  scale_colour_manual(values=c("Control" = "blue", "Drought - Resistance" = "orange")) +
...
e<-ggplot(data=SGScaps,aes(x=V1,y=V2,group=factor(Treatment),color=factor(Treatment))) + 
  xlab("LD1")+ylab("LD2")+
  stat_ellipse(data=SGScaps,aes(x=V1,y=V2,group=factor(Treatment),color=factor(Treatment)),level=0.6) + # add the convex hulls
  ggtitle("SGS")+
  geom_point(data=SGScaps,aes(x=V1,y=V2,group=factor(Treatment),color=factor(Treatment))) + # add the point markers
  scale_colour_manual(values=c("Control" = "blue", "Drought - Resistance" = "orange")) +
...
ggsave(filename = "CAPSfinalS.pdf",
       plot = caps,
       bg = "transparent",
...
tax_NBGLMs <- function(data.set) {
    results <- vector()
    for(i in 1:7){           #splits dataset for each taxonomic group
        print(paste(i, "of 2"))
...
write.csv(zotu_NBGLM_tax_ITS, "zotu_NBGLM_otu_ITS_KNZ_initial.csv")
write.csv(zotu_NBGLM_raw_ITS, "zotu_NBGLM_raw_ITS_KNZ.csv")
